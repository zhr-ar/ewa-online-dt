...
def vec_evaluate_episode_rtg(
    ...
    actions = torch.zeros(0, device=device, dtype=torch.float32) # each batch only has one action token.
    rewards = torch.zeros(0, device=device, dtype=torch.float32) # each batch only has one reward token.
