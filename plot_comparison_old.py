...
def filter_folders_by_seeds(folders, env_name, start_seed=1, end_seed=6):
    """
...
def collect_folders(base_paths, env_name, start_seed=1, end_seed=6):
    all_folders = []
...
for env_name in envs.keys():
    ewa_folders = collect_folders(ewa_base_paths, env_name, start_seed=1, end_seed=6)  # Adjust seed range as needed
...
for env_name in envs.keys():
    odt_folders = collect_folders(odt_base_paths, env_name, start_seed=1, end_seed=6)  # Adjust seed range as needed
    envs[env_name].append({
        'type': 'odt',
        'folders': odt_folders
    })
...
def load_event_logs(folder_path, metric_name):
    ...
    if any(metric_name.startswith(prefix) for prefix in ["evaluation/delta_layer_", "evaluation/decay_ratio_layer_", "evaluation/attraction_layer_"]):
        ...
        print(f"EWA specific metric: steps: {steps}")
        print(f"EWA specific metric: values: {values}")
        return steps, values
    else:
