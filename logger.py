...
now = datetime.now(tz=us_eastern) # Zahra
...
class Logger:
    ...
    def create_log_path(self, variant):
        ...
        now = datetime.now(tz=us_eastern).strftime("%Y.%m.%d/%H%M") # Zahra
        exp_name = variant["exp_name"]
        env = variant["env"] # zahra ewa
        seed = variant["seed"] # zahra ewa
        prefix = variant["save_dir"]
        # return f"{prefix}/{now}-{exp_name}"
        return f"{prefix}/{now}_{exp_name}_{env}_seed_{seed}"
